...
export class RegisterPage {
  ...
  finish(){
    if (this.show === 2) {
      ...
      this.userTatto.ageExp = parseInt(this.registerFinish.controls['ageExp'].value)
      this.userTatto.phone = parseInt(this.registerFinish.controls['phone'].value)
      this.userTatto.urlFacebook = this.registerFinish.controls['urlFacebook'].value
      this.userTatto.urlInstagram = this.registerFinish.controls['urlInstagram'].value
  ...
  userRegister(data){
    ...
    .then(res =>{
      ...
      if (res.data.code === 200) {
        ...
        this.alertCtrl.showAlert(null,'usuario ya se encuentra registado','Cerrar')
      }
      console.log(res)
    })
    .catch(e => {
